...
// Note: blockIdx = 0 only
__global__ void scalar_harris2(float *Mat, int ind_x, int ind_y, int Len, int Size, float *resMat) {
    size_t tidx = threadIdx.x;
    __shared__ float auxl[BlockSize];

    size_t stride = Len / blockDim.x + 1;
    size_t off = tidx * stride;
    size_t locx = ind_x * Len + off;
    size_t locy = ind_y * Len + off;

    float sum = 0;
    for (size_t i = 0; i < stride && off + i < Len; ++i) {
        sum += Mat[locx] * Mat[locy];
        ++locx; ++locy;
    }
    ...
    for (size_t s = 1; s < blockDim.x; s *= 2) {
        size_t id = 2 * s * tidx;
        if (id < blockDim.x) auxl[id] += auxl[id + s];
        __syncthreads();
    }

    if (tidx == 0) resMat[ind_x * Size + ind_y] = auxl[0];
}


__device__ void warp_reduce(float *auxl, size_t tidx) {
    auxl[tidx] += auxl[tidx + 32];
    auxl[tidx] += auxl[tidx + 16];
    auxl[tidx] += auxl[tidx + 8];
    auxl[tidx] += auxl[tidx + 4];
    auxl[tidx] += auxl[tidx + 2];
    auxl[tidx] += auxl[tidx + 1];
}

// <Unroll last warp>
// Note: blockIdx = 0 only
__global__ void scalar_harris5(float *Mat, int ind_x, int ind_y, int Len, int Size, float *resMat) {
    size_t tidx = threadIdx.x;
    __shared__ float auxl[BlockSize];
    ...
    for (size_t s = blockDim.x / 2; s > 32; s >>= 1) {
        if (tidx < s) auxl[tidx] += auxl[tidx + s];
        __syncthreads();
    }
    if (tidx < 32) warp_reduce(auxl, tidx);
...
// Note: blockIdx = 0 only
__global__ void scalar_harris3(float *Mat, int ind_x, int ind_y, int Len, int Size, float *resMat) {
    size_t tidx = threadIdx.x;
    __shared__ float auxl[BlockSize];
