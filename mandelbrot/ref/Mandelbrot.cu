...
int main(int argc, char **argv) {
    ...
    // cudaMandelbrot_1D<<<numBlocks,threadsPerBlock,0>>>(X0,Y0,X1,Y1,POZ,PION,ITER,Mandel_device);
    // cudaMandelbrot_2D<<<numBlocks,threadsPerBlock,0>>>(X0,Y0,X1,Y1,POZ,PION,ITER,Mandel_device);
 

    computeMandelbrot_2D(X0,Y0,X1,Y1,POZ,PION,ITER,Iters);

    // kopiujemy wyniki na CPU
